...
I'm developing the game within the [Unreal Game Engine](https://www.unrealengine.com/en-US) and use Unreal's Blueprints as to program the game logic.
...
This is accomplished by first casting rays around a character to detect overlapping sprites. The preset order of these static sprites dictate what Z position the character should be the sprites ordering value and by the character's Y position relative to the sprites explicit 'ground connection' line which tell the game system where a sprite connects with the ground and where the character should be able to walk around.
